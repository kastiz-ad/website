...
const demoTravelProfile = { preferred_name: "Demo Traveler", home_city_region: "Seoul", departure_airports: ["ICN"], arrival_airports: ["HND", "NRT"], airlines: ["Korean Air", "ANA"], hotel_brands: ["Hilton", "Hyatt"], seat_location: "window", cabin_preference: "economy", meal_preference: "Vegetarian optional", dietary_restrictions: ["No seafood on request"], accessibility_requirements: [], room_type: "Quiet room", bed_type: "King", preferred_transport: ["Train", "Taxi"], travel_pace: "balanced", preferred_currencies: ["KRW", "JPY", "USD"], direct_flight_preference: "prefer_direct", maximum_stopovers: 1, refundability_preference: "refundable_preferred", travel_purpose: "solo", price_points_strategy: "best_balance" };
const demoLoyaltyAccounts = [{ id: "demo-loyalty-1", program: "DEMO Airline Rewards", provider: "demo_airline", program_category: "airline", masked_membership_number: "DEMO-••••-0000", tier: "Preview", verification_status: "demo_only" }];
const demoPaymentMethods = [{ id: "demo-payment-1", display_name: "DEMO wallet reference", masked_label: "DEMO Pay · setup required", method_category: "other_approved_provider", connection_status: "demo_only", supported_transaction_types: ["authorize"], unavailable: true }];
const demoPaymentProviders = [{ label: "Toss Payments", category: "payment", productionStatus: "setup_required", noRealMoney: true }, { label: "Kakao Pay", category: "payment", productionStatus: "partner_application_required", noRealMoney: true }, { label: "Apple Pay", category: "device wallet", productionStatus: "partner_application_required", noRealMoney: true }];
const demoProviderCatalog = [
  { id: "google", displayName: "Google", category: "identity_login", productionConfigurationStatus: "setup_required", oauthSupported: true, secureHandoffSupported: true, allowedDomains: ["accounts.google.com"], dataScopes: ["openid", "profile", "email"], passwordCollectionAllowed: false },
  { id: "kakao", displayName: "Kakao", category: "identity_login", productionConfigurationStatus: "setup_required", oauthSupported: true, secureHandoffSupported: true, allowedDomains: ["kauth.kakao.com"], dataScopes: ["profile", "email"], passwordCollectionAllowed: false },
  { id: "booking_com", displayName: "Booking.com", category: "consumer_platform_handoff", productionConfigurationStatus: "handoff_only", oauthSupported: false, secureHandoffSupported: true, allowedDomains: ["www.booking.com"], dataScopes: ["booking.handoff"], passwordCollectionAllowed: false },
  { id: "expedia_rapid", displayName: "Expedia Rapid", category: "travel_inventory_booking", productionConfigurationStatus: "partner_application_required", oauthSupported: false, secureHandoffSupported: true, allowedDomains: ["expedia.com"], dataScopes: ["booking.handoff"], passwordCollectionAllowed: false }
];
...
function statusLabel(value) {
  ...
  if (normalized.includes("partner")) return copy("partnerRequired");
  if (normalized.includes("handoff")) return copy("handoffOnly");
  if (normalized.includes("informational")) return copy("informationalOnly");
  if (normalized.includes("setup")) return copy("setupRequired");
  if (normalized.includes("demo")) return copy("demoCapability");
  return String(value || copy("setupRequired")).replaceAll("_", " ");
}
function renderConnections(connections = [], providers = []) {
  const list = document.getElementById("connectionList");
  const catalog = document.getElementById("providerCatalog");
  list.textContent = ""; catalog.textContent = "";
  if (!connections.length) { const p = document.createElement("p"); p.className = "page-status"; p.textContent = copy("noConnections"); list.append(p); }
  connections.forEach(connection => {
    const card = document.createElement("article"); card.className = "wallet-item";
    const title = document.createElement("strong"); title.textContent = `${connection.display_name || connection.provider_id} · ${connection.status}`;
    const meta = document.createElement("span"); meta.textContent = `${connection.connection_type || copy("handoffOnly")} · ${(connection.scopes || []).join(", ") || copy("setupRequired")}`;
    const remove = document.createElement("button"); remove.type = "button"; remove.textContent = copy("remove"); remove.disabled = connection.status === "REVOKED"; remove.addEventListener("click", async () => { try { await api(`/connections/${encodeURIComponent(connection.id)}/disconnect`, { method: "POST", body: JSON.stringify({}) }); setStatus(copy("referenceRevoked")); await loadConnections(); } catch (error) { setStatus(error.message); } });
    card.append(title, meta, remove); list.append(card);
  });
  providers.forEach(provider => {
    const card = document.createElement("article"); card.className = "comparison-card provider-card";
    const h = document.createElement("h3"); h.textContent = provider.displayName;
    const p = document.createElement("p"); p.textContent = `${provider.category} · ${statusLabel(provider.productionConfigurationStatus)}`;
    const small = document.createElement("small"); small.textContent = provider.passwordCollectionAllowed ? copy("unavailable") : copy("providerPasswordWarning");
    const row = document.createElement("div"); row.className = "inline-actions";
    const connect = document.createElement("button"); connect.type = "button"; connect.textContent = provider.oauthSupported ? copy("connectProvider") : copy("learnMore") || copy("connectProvider"); connect.disabled = !provider.oauthSupported || passState.demoPreview; connect.title = passState.demoPreview ? copy("demoPreviewMode") : ""; connect.addEventListener("click", async () => { try { await api(`/connections/${encodeURIComponent(provider.id)}/authorize`, { method: "POST", body: JSON.stringify({ redirectUri: location.origin + "/one-pass.html", scopes: provider.dataScopes || [] }) }); setStatus(copy("connectionPrepared")); await loadConnections(); } catch (error) { setStatus(error.message); } });
    const handoff = document.createElement("button"); handoff.type = "button"; handoff.textContent = copy("continueHandoff"); handoff.disabled = passState.demoPreview || !provider.secureHandoffSupported || !(provider.allowedDomains || []).length; handoff.title = passState.demoPreview ? copy("demoPreviewMode") : ""; handoff.addEventListener("click", async () => { try { await api("/handoffs", { method: "POST", body: JSON.stringify({ providerId: provider.id, targetUrl: `https://${provider.allowedDomains[0]}/`, returnUrl: location.origin + "/one-pass.html", safeSummary: { demo: true, handoffReason: "user_requested" } }) }); setStatus(copy("handoffPrepared")); await loadHandoffs(); } catch (error) { setStatus(error.message); } });
    row.append(connect, handoff); card.append(h, p, small, row); catalog.append(card);
  });
}
async function loadHandoffs() {
  const target = document.getElementById("handoffList");
  if (!target) return;
  target.textContent = "";
  const data = await api("/handoffs");
  const handoffs = data.handoffs || [];
  if (!handoffs.length) { const p = document.createElement("p"); p.className = "page-status"; p.textContent = copy("noHandoffs"); target.append(p); return; }
  handoffs.forEach(handoff => { const card = document.createElement("article"); card.className = "wallet-item"; const title = document.createElement("strong"); title.textContent = `${handoff.provider_id} · ${handoff.status}`; const meta = document.createElement("span"); meta.textContent = `${handoff.mission_id || "mission"} · ${handoff.expires_at ? new Date(handoff.expires_at).toLocaleString(language) : copy("setupRequired")}`; card.append(title, meta); target.append(card); });
}
async function loadConnections() { const data = await api("/connections"); renderConnections(data.connections || [], data.availableProviders || []); await loadHandoffs(); }
function loadDemoPreview(error) {
  passState = { identityId: null, passkeyEnabled: false, passkeyConfigured: false, demoPreview: true };
  document.getElementById("identityStatus").textContent = copy("setupRequired");
  document.getElementById("travelStatus").textContent = copy("previewOnly");
  document.getElementById("loyaltyStatus").textContent = `1 ${copy("programs")}`;
  document.getElementById("paymentStatus").textContent = `1 ${copy("methods")}`;
  document.getElementById("securityStatus").textContent = copy("passkeyOff");
  document.getElementById("readyStatus").textContent = copy("previewOnly");
  document.querySelector(".pass-readiness")?.classList.remove("ready");
  document.getElementById("maskedPassport").textContent = copy("setupRequired");
  fillTravel(demoTravelProfile);
  renderLoyalty(demoLoyaltyAccounts);
  renderPayments(demoPaymentMethods, demoPaymentProviders);
  renderConnections([], demoProviderCatalog);
  const handoffList = document.getElementById("handoffList");
  if (handoffList) { handoffList.textContent = ""; const p = document.createElement("p"); p.className = "page-status"; p.textContent = copy("noHandoffs"); handoffList.append(p); }
  const activity = document.getElementById("activityList");
  if (activity) { activity.textContent = ""; const li = document.createElement("li"); const label = document.createElement("span"); const time = document.createElement("time"); label.textContent = copy("demoPreviewMode"); time.textContent = new Date().toLocaleString(language); li.append(label, time); activity.append(li); }
  setStatus(`${copy("demoPreviewMode")} ${error?.message || ""}`.trim());
...
async function load() {
  try {
    ...
    passState = { identityId: data.status.identity?.id || null, passkeyEnabled: Boolean(data.status.passkeyEnabled), passkeyConfigured: Boolean(data.status.security?.passkeyConfigured), demoPreview: false };
    ...
    await loadLoyalty(); await loadPayments(); await loadConnections(); await loadActivity(); configureSecurityButtons();
  } catch (error) { loadDemoPreview(error); }
}
