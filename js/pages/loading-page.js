...
import { resolveDestinationEntities } from "../engine/world/global-entity-resolver.js?v=20260916-tourism-relevance-v29";
...
import { enrichNamedEntityMedia, summarizeMediaEnrichment } from "../engine/media/global-content-media-enrichment.js?v=20260916-tourism-relevance-v29";
...
const fetchLocalPlaces = async (mission) => {
  ...
  try {
    ...
    const normalize = (entry) => {
      ...
      const commonsFile = String(tags.wikimedia_commons || "").replace(/^File:/i, ""); const imageUrl = tags.image || (commonsFile ? "https://commons.wikimedia.org/wiki/Special:FilePath/" + encodeURIComponent(commonsFile) + "?width=1200" : ""); const images = [tags.image ? { url: tags.image, source: "OpenStreetMap", entityLinked: true } : null, commonsFile ? { url: `https://commons.wikimedia.org/wiki/Special:FilePath/${encodeURIComponent(commonsFile)}?width=1200`, source: "Wikimedia Commons", entityLinked: true } : null].filter(Boolean); return stampGeographicEvidence({ label: name, value: tags.tourism || tags.amenity || tags.historic || tags.leisure || tags.public_transport || "place", kind, cuisine: tags.cuisine || "", stars: tags.stars || "", rating: null, address: [tags["addr:housenumber"], tags["addr:street"], tags["addr:city"]].filter(Boolean).join(" "), website: tags.website || tags["contact:website"] || "", publicEntityUrl: tags.url || "", imageUrl, images, imageAlt: name, wikipedia: tags.wikipedia || "", wikidata: tags.wikidata || "", providerId: `${entry.type}/${entry.id}`, osmId: `${entry.type}/${entry.id}`, source: "OpenStreetMap" }, geographicScope, { latitude: entry.lat || entry.center?.lat, longitude: entry.lon || entry.center?.lon });
    };
    ...
    if (items.filter((item) => item.kind === "restaurant").length < 4) {
      ...
      (Array.isArray(restaurantSearch) ? restaurantSearch : []).forEach((place) => {
        ...
        items.push(stampGeographicEvidence({ label, value: place.type || "restaurant", kind: "restaurant", cuisine: place.extratags?.cuisine || "", stars: "", rating: null, address: place.display_name || "", website: place.extratags?.website || place.extratags?.["contact:website"] || "", publicEntityUrl: place.extratags?.url || "", imageUrl: place.extratags?.image || (commonsFile ? `https://commons.wikimedia.org/wiki/Special:FilePath/${encodeURIComponent(commonsFile)}?width=1200` : ""), images: [place.extratags?.image ? { url: place.extratags.image, source: "OpenStreetMap Nominatim", entityLinked: true } : null, commonsFile ? { url: `https://commons.wikimedia.org/wiki/Special:FilePath/${encodeURIComponent(commonsFile)}?width=1200`, source: "Wikimedia Commons", entityLinked: true } : null].filter(Boolean), wikipedia: place.extratags?.wikipedia || "", wikidata: place.extratags?.wikidata || "", providerId: `${place.osm_type}/${place.osm_id}`, osmId: `${place.osm_type}/${place.osm_id}`, source: "OpenStreetMap Nominatim" }, geographicScope, { latitude: place.lat, longitude: place.lon }));
      });
    }
    if (items.filter((item) => item.kind === "hotel").length < 5) {
      ...
      (Array.isArray(accommodationSearch) ? accommodationSearch : []).forEach((place) => {
        ...
        seen.add(key);
        const commonsFile = String(place.extratags?.wikimedia_commons || "").replace(/^File:/i, "");
        items.push(stampGeographicEvidence({ label, value: place.type || "hotel", kind: "hotel", cuisine: "", stars: place.extratags?.stars || "", rating: null, address: place.display_name || "", website: place.extratags?.website || place.extratags?.["contact:website"] || "", publicEntityUrl: place.extratags?.url || "", imageUrl: place.extratags?.image || (commonsFile ? `https://commons.wikimedia.org/wiki/Special:FilePath/${encodeURIComponent(commonsFile)}?width=1200` : ""), images: [place.extratags?.image ? { url: place.extratags.image, source: "OpenStreetMap Nominatim", entityLinked: true } : null, commonsFile ? { url: `https://commons.wikimedia.org/wiki/Special:FilePath/${encodeURIComponent(commonsFile)}?width=1200`, source: "Wikimedia Commons", entityLinked: true } : null].filter(Boolean), wikipedia: place.extratags?.wikipedia || "", wikidata: place.extratags?.wikidata || "", providerId: `${place.osm_type}/${place.osm_id}`, osmId: `${place.osm_type}/${place.osm_id}`, source: "OpenStreetMap Nominatim" }, geographicScope, { latitude: place.lat, longitude: place.lon }));
      });
    }
  ...
  try {
    enrichedMission = await Promise.race([
      ...
      wait(18000).then(() => ({
        ...mission,
        status: "mission_ready",
        providerResults: [fallbackProvider("ONE Safe Loader", "mission", "Mission preparation timed out safely. A useful preview is still available; live provider data was not fabricated.", "loading_timeout")],
        placeIntelligence: { fallback: { AI_REASONING: true }, blankResultsAllowed: false },
        approvalProtection: { required: true, message: "No booking, payment, provider contact, or submission occurs without explicit approval." },
        updatedAt: new Date().toISOString()
      }))
    ]);
  } catch (error) {
    enrichedMission = {
      ...mission,
      status: "mission_ready",
      providerResults: [fallbackProvider("ONE Safe Loader", "mission", "Mission preparation recovered from a provider error. A useful preview is still available; live provider data was not fabricated.", error?.message || "loading_error")],
      placeIntelligence: { fallback: { AI_REASONING: true }, blankResultsAllowed: false },
      approvalProtection: { required: true, message: "No booking, payment, provider contact, or submission occurs without explicit approval." },
      updatedAt: new Date().toISOString()
    };
