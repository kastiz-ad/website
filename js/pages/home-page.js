...
import { classifyMission, destinationAwareMissionType } from "../engine/mission-classification.js?v=20260916-global-entity-semantics-v27";
import { detectWorldwideTravelDestination } from "../ui/mission-followup.js?v=20260916-global-entity-semantics-v27";
...
import { applyTravelConstraints, parseTravelConstraints } from "../engine/travel/travel-constraint-parser.js?v=20260907-founder-qa-v20";
...
const reopenPrototypeMission = (reference) => {
  try {
    ...
    location.href = `results.html?savedTrip=1&lang=${encodeURIComponent(activeLanguage)}`;
    return true;
  } catch {
    return false;
  }
...
const saveMission = (mission, schedule = null) => {
  ...
  if (["presentation", "meeting", "interview"].includes(classifiedType)) {
    ...
    payload.missionType = classifiedType;
    payload.domain = "work";
    payload.missionCategory = "work";
    payload.missionFoundation = workFoundation;
    payload.missionBriefing = buildMissionBriefing(workFoundation);
  }
