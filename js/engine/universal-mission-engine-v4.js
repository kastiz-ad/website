...
export function buildUniversalMission(input={}){
  ...
  return {...experience,pipeline:PIPELINE,classification,humanReasoning,lifeMemoryContext,contextObject,futureMissionSuggestions,providerType:classification.providerType,providers:[experience.onePick,...experience.alternatives].map(x=>x.providerData||normalizeProvider({providerId:x.id,providerType:classification.providerType,businessName:x.title,dataState:"demo"})),liveIntelligence,transit,preparation,localMissionPlan,whyNotOthers:experience.alternatives.map(x=>({id:x.id,title:x.title,tradeoff:x.tradeoff}))};
}
