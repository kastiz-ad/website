...
import { buildHumanReasoningObject, HUMAN_REASONING_PIPELINE_SLOT } from "./human-reasoning/human-reasoning-engine.js";
import { buildLocalMissionPlan, classifyLocalMission, isLocalMissionType } from "./local-mission-platform/local-mission-platform.js";
...
function universalOptions(type,language="en",input={}){
  if(isLocalMissionType(type)){
    const plan=buildLocalMissionPlan({...input,type,language});
    return plan.providerLayer.rankedProviders.slice(0,3).map((provider,index)=>({id:provider.id,title:provider.name||provider.businessName,identity:index===0?(language==="ko"?"종합 추천":"Best Overall"):index===1?(language==="ko"?"가까운 후보":"Nearby Option"):(language==="ko"?"유연한 후보":"Flexible Option"),cost:provider.estimatedCost||provider.price||(language==="ko"?"승인 전 확인":"Confirm before approval"),time:provider.availability||"To confirm",score:provider.score,reasons:provider.matchReasons,tradeoff:plan.fallbackPolicy,itinerary:[{time:language==="ko"?"준비":"Prepare",title:language==="ko"?"필요 조건 확인":"Confirm requirements",duration:"5 min",travelMinutes:0,reservation:false,cost:"—",note:language==="ko"?"제공업체에 연락하지 않음":"No provider contact"},{time:language==="ko"?"비교":"Compare",title:language==="ko"?"후보와 조건 비교":"Compare provider fit",duration:"5 min",travelMinutes:0,reservation:false,cost:"—",note:provider.source||"Prototype fallback"},{time:language==="ko"?"승인":"Approval",title:language==="ko"?"명시적 승인 대기":"Wait for explicit approval",duration:language==="ko"?"사용자가 결정":"User controlled",travelMinutes:0,reservation:false,cost:"—",note:language==="ko"?"실행 비활성화":"Execution disabled"}],transport:{mode:language==="ko"?"위치 기반 이동 확인":"Location-based route check",evidence:language==="ko"?"정확한 위치와 승인 전까지 예상 단계입니다.":"Estimated until exact location and approval are confirmed.",estimateLabel:language==="ko"?"예상":"Estimated"},weatherBackup:null,nearby:[],value:{baselineValid:false,convenience:language==="ko"?["하나의 후보 비교","하나의 승인 단계","실행 전 보호"]:["One shortlist","One approval checkpoint","Protected before execution"]},providerData:normalizeProvider({providerId:provider.id,providerType:type,businessName:provider.name||provider.businessName,availability:provider.availability,estimatedCost:provider.estimatedCost,providerSource:provider.source||"Prototype provider layer",dataState:"demo",specificFields:{service:provider.service,score:provider.score,matchReasons:provider.matchReasons}}),rating:null}));
  }
...
export function buildUniversalMission(input={}){
  const rawClassification=classifyUniversalMission(input.mission),purposeType={"korea-weekend":"travel","global-escape":"travel",business:"travel",event:"entertainment",healthcare:"healthcare"}[input.purpose];const classification=purposeType?{...rawClassification,providerType:purposeType,confidence:"context-supported"}:rawClassification,language=input.language||"en",context=buildExperienceContext({...input,purpose:classification.providerType==="travel"?undefined:classification.providerType}),humanReasoning=buildHumanReasoningObject({...input,mission:classification.mission,classification,language,context}),lifeMemoryContext=buildLifeMemoryContext({memory:input.lifeMemory,missionType:classification.providerType,explicitInstructions:classification.mission,language}),aiMissionMemoryContext=buildMissionPersonalization(input.aiMissionMemory||input.missionMemory||{},{missionType:classification.providerType,explicitInstructions:classification.mission,language}),contextObject=buildContextObject({...input,mission:classification.mission,classification,providerType:classification.providerType,missionType:classification.providerType,language}),futureMissionSuggestions=generateFutureMissionSuggestions({...input,mission:classification.mission,classification,providerType:classification.providerType,missionType:classification.providerType,language,contextObject});
  let experience;
  ...
  const localMissionPlan=isLocalMissionType(classification.providerType)?buildLocalMissionPlan({...input,type:classification.providerType,language}):null;
  return {...experience,pipeline:PIPELINE,classification,humanReasoning,lifeMemoryContext,aiMissionMemoryContext,contextObject,futureMissionSuggestions,providerType:classification.providerType,providers:[experience.onePick,...experience.alternatives].map(x=>x.providerData||normalizeProvider({providerId:x.id,providerType:classification.providerType,businessName:x.title,dataState:"demo"})),liveIntelligence,transit,preparation,localMissionPlan,whyNotOthers:experience.alternatives.map(x=>({id:x.id,title:x.title,tradeoff:x.tradeoff}))};
}
