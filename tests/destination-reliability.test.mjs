...
assert.match(followup, /Choose the exact destination/);
...
assert.match(followup, /destinationResolutionDecision\?\.selected/);
assert.match(followup, /Several real places share that name/);
assert.doesNotMatch(followup, /resolvedDestination = matches\[0\]/);
assert.doesNotMatch(followup, /Several places share this name\. Choose the exact place and country\./);
assert.match(followup, /showResolvedDestination\(resolvedDestination\)/);
...
assert.match(loading, /네이버 지도·대중교통/);
...
assert.match(followup, /모르면 비워두세요\. ONE이 여러 가격대를 비교할게요\./);
...
assert.match(results, /providerSource: source \|\| "ONE World Intelligence estimate"/);
assert.match(indexHtml, /script\.js\?v=20260811-private-investor-entry-v1/);
assert.match(indexHtml, /style\.css\?v=20260810-investor-demo-polish-2/);
...
assert.match(loadingHtml, /loading\.js\?v=20260811-results-localization-v1/);
...
assert.match(followup, /geocoding-api\.open-meteo\.com\/v1\/search/);
assert.match(followup, /new Intl\.DisplayNames\(\["es"\]/);
assert.match(followup, /item\.countryEs/);
assert.match(followup, /spanishPrefixPhrase/);
assert.match(followup, /destinationQueriesFromMission/);
assert.match(home, /type === "travel" \|\| type === "general_mission"/);
...
assert.match(results, /else if \(currentResult\?\.portableShare === true\)/);
assert.match(results, /renderV231BlockedCompletionState\(\)/);
...
assert.match(results, /priceBasis: worldFlight\.priceState \|\| "estimated_until_live_search"/);
...
assert.doesNotMatch(results, /accommodation Ã¢â‚¬â€ live provider confirmation required/);
assert.doesNotMatch(results, /central hotel Ã¢â‚¬â€ provider confirmation required/);
...
assert.match(results, /v23-selected-journey/);
