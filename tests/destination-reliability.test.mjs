...
assert.match(loading, /ì‹ë‹¹ ë„¤ì´ë²„ ì˜ˆì•½/);
assert.match(results, /createPublicResourceCard/);
assert.match(followup, /How much can you spend\? \(optional\)/);
assert.match(followup, /ì´ ì˜ˆì‚°ì€ ì–¼ë§ˆì¸ê°€ìš”\?/);
assert.match(followup, /ì•„ì§ ëª¨ë¥´ê² ë‹¤ë©´ ë¹„ì›Œë‘ì„¸ìš”\. ONEì´ ë‹¤ì–‘í•œ ê°€ê²©ëŒ€ë¥¼ ë¹„êµí•´ë“œë ¤ìš”\./);
assert.match(followup, /Best overall balance/);
assert.match(followup, /ê°€ê²©Â·í’ˆì§ˆÂ·ì‹œê°„ì„ ê³ ë¥´ê²Œ/);
...
assert.match(resultsHtml, /results\.js\?v=20260727-alpha03-visualization/);
assert.match(resultsHtml, /location\.replace\(u\.href\)/);
assert.match(resultsHtml, /no-cache, no-store, must-revalidate/);
assert.match(resultsHtml, /results\.css\?v=20260727-alpha03-visualization/);
...
assert.match(results, /preparedByOne: "ONE ì´ ì¤€ë¹„í–ˆìŠµë‹ˆë‹¤\."/);
assert.match(results, /flightEstimateNotice: "Estimated price range Â· not a live fare"/);
...
assert.doesNotMatch(results, /accommodation â€” live provider confirmation required/);
assert.doesNotMatch(results, /central hotel â€” provider confirmation required/);
