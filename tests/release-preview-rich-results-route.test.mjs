...
test("rich travel renderer still includes populated result sections", () => {
  const requiredPatterns = [
    ...
    /alpha03-map-pin/,
    /<img data-safe-travel-image\$\{fallbackAttribute\} src="\$\{escapeSummaryText\(image\.url\)\}"/,
...
test("public assets use the release cache buster for the rich preview fix", () => {
  assert.match(resultsHtml, /results\.css\?v=20260915-global-dining-media-v22/);
  assert.match(resultsHtml, /results\.js\?v=20260915-global-dining-media-v22/);
  assert.match(resultsJs, /results-page\.js\?v=20260915-global-dining-media-v22/);
