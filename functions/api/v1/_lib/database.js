...
export async function db(cfg, user, table, { method = "GET", query = "", body, prefer = "return=representation" } = {}) {
  const response = await fetchWithTimeout(`${cfg.supabaseUrl}/rest/v1/${table}${query ? `?${query}` : ""}`, {
  ...
  }, { timeoutMs: cfg.upstreamTimeoutMs, errorCode: "database_upstream_timeout" });
...
export async function trustedDb(cfg, table, { method="POST",query="",body,prefer="return=representation" }={}) {
  ...
  const response=await fetchWithTimeout(`${cfg.supabaseUrl}/rest/v1/${table}${query?`?${query}`:""}`,{method,headers:{apikey:cfg.serviceKey,Authorization:`Bearer ${cfg.serviceKey}`,"Content-Type":"application/json",Prefer:prefer},body:body===undefined?undefined:JSON.stringify(body)},{timeoutMs:cfg.upstreamTimeoutMs,errorCode:"trusted_database_upstream_timeout"});
