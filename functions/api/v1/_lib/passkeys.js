...
export class WebAuthnVerifier {
  ...
  async verify(request) {
    if (!hasWebAuthnVerifier(this.binding)) throw new ApiError(503, "passkey_verifier_not_configured", "Passkey verification service is not configured.");
    return this.binding.verify(request);
  }
}
